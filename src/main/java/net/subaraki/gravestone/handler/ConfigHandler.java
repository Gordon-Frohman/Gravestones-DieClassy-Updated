...
public class ConfigHandler
{
    ...
    private void loadSettings(final Configuration config) {
        ConfigHandler.enableGravesTroughKey = config.get("GraveStones", "enable Graves Trough Key", true).getBoolean(true);
        ConfigHandler.graveLevel = config.get("GraveStones", "change graves every x level", 5).getInt(5);
        ConfigHandler.graveOrder = config.get("GraveStones", "grave orders", new int[] { 6, 1, 2, 3, 7, 4, 5, 10, 9, 8 }).getIntList();
        ConfigHandler.allowDebug = config.get("GraveStones", "Toggle debug lines to print in the console", false).getBoolean(false);
    }
