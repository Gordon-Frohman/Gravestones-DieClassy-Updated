...
public class GravestoneEventHandler
{
    ...
    public void keyHandling(final InputEvent.KeyInputEvent evt) {
        if (ConfigHandler.enableGravesTroughKey && ClientProxy.keyGui.isPressed() && Minecraft.getMinecraft().currentScreen == null) {
            final EntityPlayer p = (EntityPlayer)Minecraft.getMinecraft().thePlayer;
            p.openGui((Object)GraveStones.instance, 1, (World)Minecraft.getMinecraft().theWorld, (int)p.posX, (int)p.posY, (int)p.posZ);
        }
    }
    ...
    public void onDeathEvent(final LivingDeathEvent evt) {
        if (evt.entityLiving instanceof EntityPlayer) {
            ...
            if (player.worldObj.isAirBlock(x, y, z)) {
                if (y < 0) {
    				//При гибели в пустоте могила должна возникнуть на уровне бедрока или выше
    				if(player.worldObj.isAirBlock(x, 0, z))
    					player.worldObj.setBlock(x, 0, z, Blocks.bedrock);
    				y = 1;
    				while(!player.worldObj.isAirBlock(x, y, z) && y < 255)
    					y++;
                }
            ...
            for (int i = 0; i < maxI; ++i) {
                if (-X / 2 <= x2 && x2 <= X / 2 && -Z / 2 <= z2 && z2 <= Z / 2) {
                    int y2;
                    for (y2 = 0; !player.worldObj.getBlock(x + x2, y + 1 + y2, z + z2).getMaterial().equals(Material.air); ++y2) {}
                    if (player.worldObj.getBlock(x + x2, y + y2, z + z2).getMaterial().isLiquid()) {
                        GraveStones.printDebugMessage("You were standing in liquid !");
                        --y2;
                        liquid = true;
                    }
                    if (player.worldObj.getBlock(x + x2, y + y2, z + z2).getMaterial().isSolid() && (player.worldObj.getBlock(x + x2, y + 1 + y2, z + z2).getMaterial().equals(Material.air) || liquid)) {
            ...
            if (!flag && liquid) {
                int y3;
                for (y3 = 0; !player.worldObj.getBlock(x, y + 1 + y3, z).getMaterial().equals(Material.air); ++y3) {}
                if (player.worldObj.getBlock(x, y + y3, z).getMaterial().isLiquid()) {
                    if (player.worldObj.getBlock(x, y + 1 + y3, z).getMaterial().equals(Material.air)) {
    ...
    private void addOtherInventory(final TileEntityGravestone te, final EntityPlayer player) {
        if (GraveStones.hasRpgI) {
            final IInventory inv = this.accesInventoryContents(player, "get", "rpgInventory.gui.rpginv.PlayerRpgInventory", "Rpg Inventory");
            if (inv != null) {
                for (int i = 0; i < 7; ++i) {
        ...
        if (GraveStones.hasBaub) {
            final IInventory inv = this.accesInventoryContents(player, "getPlayerBaubles", "baubles.common.lib.PlayerHandler", "Baubles");
            if (inv != null) {
                for (int i = 0; i < 4; ++i) {
                    final ItemStack is = inv.getStackInSlot(i);
                    te.list[i + 81] = is;
        ...
        if (GraveStones.hasGal_Craft) {
            final ItemStack[] inv3 = this.accesInventoryContentsStacks(player, "getExtendedInventory", "micdoodle8.mods.galacticraft.core.inventory.InventoryExtended", "GalactiCraft");
            for (int i = 0; i < 10; ++i) {
        ...
        if (GraveStones.hasMari_Cul) {
            final ItemStack[] inv3 = this.accesInventoryContentsStacks(player, "getInventory", "mariculture.magic.MirrorHelper", "Mariculture");
            for (int i = 0; i < 3; ++i) {
                final ItemStack is = inv3[i];
                te.list[i + 95] = is;
            }
    ...
    private IInventory accesInventoryContents(final EntityPlayer player, final String methodName, final String path, final String modName) {
        try {
    ...
    private ItemStack[] accesInventoryContentsStacks(final EntityPlayer player, final String methodName, final String path, final String modName) {
        try {
            final Class<?> clazz = Class.forName(path);
            final Method m = clazz.getDeclaredMethod(methodName, EntityPlayer.class);
            final Object result = m.invoke(null, player);
    ...
    private IInventory accesInventoryContents(final EntityPlayer player, final String methodName, final String path, final String declaredField, final String modName) {
        try {
            final Class<?> clazz = Class.forName(path);
            final Method m = clazz.getDeclaredMethod(methodName, EntityPlayer.class);
            final Object result = m.invoke(null, player);
