...
import net.subaraki.gravestone.integration.ForgeCreeperHealIntegration;
...
public class GravestoneEventHandler {
    ...
    public void onDeathEvent(LivingDeathEvent evt) {
        if (evt.entityLiving instanceof EntityPlayer player) {
            ...
            this.scheduleEvent(() -> { findPlaceForGrave(world, player, x, y, z); }, 10);
        }
    }

    private boolean isAirBlock(World world, int x, int y, int z) {
        boolean result = world.isAirBlock(x, y, z);

        if (!world.isRemote && result && GraveStones.hasForgeCreeperHeal)
            return !ForgeCreeperHealIntegration.isBlockGoingToHeal((WorldServer) world, x, y, z);
    ...
    private void findPlaceForGrave(World world, EntityPlayer player, int x, int y, int z) {
        if (isAirBlock(world, x, y, z)) {
            ...
            while (isAirBlock(world, x, y, z)) --y;
        }
        int X = 100;
        int Z = 100;
        int x2 = 0;
        ...
        for (int i = 0; i < maxI; ++i) {
            if (-X / 2 <= x2 && x2 <= X / 2 && -Z / 2 <= z2 && z2 <= Z / 2) {
                int y2;
                for (y2 = 0; !world.getBlock(x + x2, y + 1 + y2, z + z2)
                    .getMaterial()
                    .equals(Material.air); ++y2) {}
                if (world.getBlock(x + x2, y + y2, z + z2)
                    .getMaterial()
                    .isLiquid()) {
                ...
                if (world.getBlock(x + x2, y + y2, z + z2)
                    .getMaterial()
                    .isSolid()
                    && (world.getBlock(x + x2, y + 1 + y2, z + z2)
                        .getMaterial()
        ...
        if (!flag && liquid) {
            int y3;
            for (y3 = 0; !world.getBlock(x, y + 1 + y3, z)
                .getMaterial()
                .equals(Material.air); ++y3) {}
            if (world.getBlock(x, y + y3, z)
                .getMaterial()
                .isLiquid()) {
                if (world.getBlock(x, y + 1 + y3, z)
                    .getMaterial()
                    .equals(Material.air)) {
                    world.setBlock(x, y + y3, z, Blocks.cobblestone);
                }
    ...
    private void scheduleEvent(Runnable event, int delay) {
        if (delay > 0) scheduledEvents.add(new ScheduledEvent(delay, event));
        else event.run();
    }
